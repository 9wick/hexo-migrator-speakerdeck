import {getSpeakerDeckJson} from "./speakerdeck";
...
export const migrator = async function (this: Hexo, args: any) {
  ...
  let feed: PromiseType<ReturnType<typeof getSpeakerDeckJson>>


  try {
    if (!source) {
      const help = [
        'Usage: hexo migrate speakerdeck <username> [--alias]',
        '',
    ...
    feed = await getSpeakerDeckJson(source);
